defmodule Day18.Part2 do
  alias Day18.Part2.Operations

  def main(input) do
    Day18.Part2.read_file(input)
    |> compute_all(0)
    |> IO.inspect()
  end

  def compute_all({_, []}, max), do: max

  def compute_all({map, [first | rest]}, max) do
    max =
      Enum.max([
        compute_a_with_all(map, first, rest, max),
        compute_all_with_a(map, first, rest, max)
      ])

    compute_all({map, rest}, max)
  end

  def compute_a_with_all(_, _, [], max), do: max

  def compute_a_with_all(map, a, [b | rest], max) do
    result = Operations.add(map, a, b)
    compute_a_with_all(map, a, rest, Enum.max([max, result]))
  end

  def compute_all_with_a(_, _, [], max), do: max

  def compute_all_with_a(map, a, [b | rest], max) do
    result = Operations.add(map, b, a)
    compute_all_with_a(map, a, rest, Enum.max([max, result]))
  end
end

defmodule Day18.Part2.Operations do
  alias Day18.Pair
  ...
  def add(map, a, b) do
    ...
    reduce(map, new_pair.id) |> compute_magnitude()
  end
...
defmodule Day18.Part2.Parsing do
  alias Day18.Pair
